        ...
        <>
          ...
          <div className="grid two">
            ...
            <Card title="LLM Provider" description="不展示任何 API Key。">
              {providers?.providers.length ? <div className="cards-list">{providers.providers.map((provider) => <div className="item-card" key={provider.name} style={{ '--accent': provider.configured ? 'var(--green)' : 'var(--orange)' } as React.CSSProperties}><div className="meta"><Badge text={provider.name} tone={provider.configured ? 'green' : 'orange'} /><span>{provider.configured ? '已配置' : '未配置'}</span></div><p><strong>模型：</strong>{provider.model}</p><p><strong>Base URL：</strong>{provider.base_url}</p></div>)}</div> : <Empty text="后端返回空 Provider 列表" />}
            </Card>
          </div>
